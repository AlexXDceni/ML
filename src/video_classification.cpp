...
ExternalScreen screen(RESOLUTION, RESOLUTION);

NeuralNetwork nn({2, 16, 16, 1}, "sigmoid"); 

void clasificare_puncte(bool isEmptyDataSet = false)
{
    cout << "========================================================\n";
...
void fractal()
{
    nn = NeuralNetwork ({2, 64, 64, 3}, "sigmoid"); 
    
    vector<vector<vector<double>>> harta_fractal(RESOLUTION, vector<vector<double>>(RESOLUTION, vector<double>(3, 0.0)));

        const double c_real = -0.7;
        const double c_imag = 0.27015;
        const int MAX_ITERATII = 60;
        ...
        for (int y = 0; y < RESOLUTION; y++) {
            for (int x = 0; x < RESOLUTION; x++) {
                ...
                while (z_real * z_real + z_imag * z_imag < 4.0 && iteratie < MAX_ITERATII) {
                ...
                double mu = (double)iteratie / MAX_ITERATII;
                harta_fractal[y][x][0] = std::sin(mu * 3.14159 / 2.0); 
                harta_fractal[y][x][1] = mu * mu;                     
                harta_fractal[y][x][2] = std::sqrt(mu);                
            }
        ...
        nn.data.targets.clear();
            for (int i = 0; i < 8000; i++) {
               ...
               nn.data.inputs.push_back({ nx, ny });
                nn.data.targets.push_back({ harta_fractal[ry][rx][0], harta_fractal[ry][rx][1], harta_fractal[ry][rx][2] });
        }
