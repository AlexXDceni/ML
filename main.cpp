...
public:
    ...
    void addPhotoToTraining(string filename, double target, TrainingData& data) {
        ...
        if (magic == "P6") { // Daca e PPM Color (ceea ce ai tu acum)
            for (int i = 0; i < w * h; ++i) {
                unsigned char r, g, b;
                file.read((char*)&r, 1);
                file.read((char*)&g, 1);
                file.read((char*)&b, 1);
                // Transformam RGB in Grayscale folosind formula de luminozitate
                double gray = (0.299 * r + 0.587 * g + 0.114 * b) / 255.0;
                pixels.push_back(gray);
            }
        } 
        else if (magic == "P5") { // Grayscale Binary
            for (int i = 0; i < w * h; ++i) {
                unsigned char pixel;
        ...
        else if (magic == "P2") { // Grayscale ASCII
            for (int i = 0; i < w * h; ++i) {
                int val; file >> val;
        ...
        if (pixels.size() == 784) {
            ...
            if(target >= 0 && target < 10) targetVector[(int)target] = 1.0;
...
int main()
{
    ...
    Network nn = Network({input, 16, 32, 16, output});
    ...
    case 1: // Predict
        ...
        for (int i = 0; i < data.inputs.size(); i++)
        {
            double prediction = nn.predictBiggest(data.inputs[i]);  //vector<double> prediction = nn.predict(data.inputs[i]);

            cout << "Input " << i << ": ";
            // for (double val : data.inputs[i])
            //     cout << val << " ";

            cout << "-> AI says: " << prediction << endl; //cout << "-> AI says: " << prediction[0] << endl; //  (prediction[0] == 1 ? "1" : "0")  //  prediction[0]
        }
        break;
    }
    return 0;
